import Link from 'next/link';
import { BUSINESS, CATEGORIES, WHATSAPP_NUMBER } from '@/lib/constants';

export default function Footer() {
  return (
    <footer className="border-t border-[#E0E0E0] bg-white">
      {/* Columnas principales */}
      <div className="wrap grid gap-10 py-16 sm:grid-cols-2 lg:grid-cols-4">
        {/* Marca */}
        <div className="lg:col-span-2">
          <div className="flex items-center gap-3">
            <span className="flex h-8 w-8 items-center justify-center bg-ink text-xs font-black text-white">
              MA
            </span>
            <span className="text-sm font-black uppercase tracking-[0.14em] text-ink">
              Moda Adam
            </span>
          </div>
          <p className="mt-1 text-xs font-bold uppercase tracking-[0.2em] text-clay">
            {BUSINESS.tagline}
          </p>

          <p className="mt-5 max-w-xs text-sm leading-relaxed text-muted">
            {BUSINESS.address}
          </p>
          <p className="mt-3 text-sm text-muted">
            Lun–Vie <span className="text-ink">{BUSINESS.hours}</span>
          </p>

          {/* WhatsApp */}
          <a
            href={`https://wa.me/${WHATSAPP_NUMBER}`}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-ink hover:text-clay"
          >
            <WhatsAppIcon />
            {BUSINESS.phone}
          </a>

          {/* Email */}
          <a
            href={`mailto:${BUSINESS.email}`}
            className="mt-2 flex items-center gap-2 text-sm text-muted hover:text-ink"
          >
            <MailIcon />
            {BUSINESS.email}
          </a>
        </div>

        {/* Catálogo */}
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-ink">Catálogo</p>
          <ul className="mt-4 space-y-2.5">
            {CATEGORIES.map((c) => (
              <li key={c.slug}>
                <Link
                  href={`/catalogo/${c.slug}`}
                  className="text-sm text-muted transition hover:text-ink"
                >
                  {c.label}
                </Link>
              </li>
            ))}
          </ul>
        </div>

        {/* Legal + cuenta */}
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-ink">Cuenta</p>
          <ul className="mt-4 space-y-2.5">
            <li>
              <Link href="/login" className="text-sm text-muted transition hover:text-ink">
                Iniciar sesión
              </Link>
            </li>
            <li>
              <Link href="/registro" className="text-sm text-muted transition hover:text-ink">
                Crear cuenta mayorista
              </Link>
            </li>
            <li>
              <Link href="/mis-pedidos" className="text-sm text-muted transition hover:text-ink">
                Mis pedidos
              </Link>
            </li>
          </ul>

          <p className="mt-8 text-[10px] font-black uppercase tracking-[0.2em] text-ink">Legal</p>
          <ul className="mt-4 space-y-2.5">
            <li>
              <span className="text-sm text-muted">Términos y condiciones</span>
            </li>
            <li>
              <span className="text-sm text-muted">Política de privacidad</span>
            </li>
            <li>
              <span className="text-sm text-muted">Aviso de cookies</span>
            </li>
          </ul>
        </div>
      </div>

      {/* Barra inferior */}
      <div className="border-t border-[#E0E0E0]">
        <div className="wrap flex flex-col items-center justify-between gap-2 py-5 text-xs text-muted sm:flex-row">
          <p>
            © {new Date().getFullYear()} Moda Adam · Ropa al por mayor · Crevillent (Alicante)
          </p>
          <p className="text-muted/60">Prototipo de demostración · Precios sin IVA</p>
        </div>
      </div>
    </footer>
  );
}

function WhatsAppIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
      <path d="M.057 24l1.687-6.163a11.867 11.867 0 0 1-1.587-5.945C.16 5.335 5.495 0 12.05 0a11.82 11.82 0 0 1 8.413 3.488 11.82 11.82 0 0 1 3.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 0 1-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 0 0 1.51 5.26l-.999 3.648 3.978-1.607zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z" />
    </svg>
  );
}

function MailIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
      <polyline points="22,6 12,13 2,6"/>
    </svg>
  );
}
